...
public class Main {
    ...
    public static void register_monitor(Config mConfig){
        byte[] buff = DataTransfer.register2Monitor(Server.SERVER_LOGIN,GServer.mServerInfo.name, GServer.mServerInfo.id,GServer.mServerInfo.host, GServer.mServerInfo.port);
    ...
    public static void unregister_monitor(Config mConfig){
        byte[] buff = DataTransfer.unregister2Monitor(Server.SERVER_LOGIN,GServer.mServerInfo.name, GServer.mServerInfo.id,GServer.mServerInfo.host, GServer.mServerInfo.port);
        int monitorSize = (null != mConfig.monitor_net_udp) ? mConfig.monitor_net_udp.length : 0;
    	if(monitorSize > 0){
    		for(int i=0; i< monitorSize ; i++){
    			NetUtil.send_data_by_udp_nio(mConfig.monitor_net_udp[i].ip, mConfig.monitor_net_udp[i].port,buff,0,DataPacket.Header.getLength(buff));
	...
	private static IConnectListener mDisPatcherConnectResultListener = new IConnectListener() {
		...
		public void onConnectionSuccess(BaseClient client) {
			...
			byte[] buff = DataTransfer.register2Dispatcher(Server.SERVER_LOGIN,GServer.mServerInfo.name, GServer.mServerInfo.id,GServer.mServerInfo.host, GServer.mServerInfo.port);
