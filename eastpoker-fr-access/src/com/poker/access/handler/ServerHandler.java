		...
		int server = cmd >> 16;
      	...
      	if(server > 0){
      		...
      		if(server == ServerIds.SERVER_LOGIN){
      			...
      			length = PacketTransfer.send2Login(mTempBuff, squenceId, uid,cmd,DistapchType.TYPE_P2P,mOutPacket.getPacket(),0,  mOutPacket.getLength());
      			...
      			//当InPacket不需要使用时，可以复用buff，防止过多的分配内存，产生内存碎片
      			byte[] mTempBuff = mInPacket.getPacket();
      			length = PacketTransfer.send2User(mTempBuff, squenceId, uid,cmd,DistapchType.TYPE_P2P,mOutPacket.getPacket(),0,  mOutPacket.getLength());
          		send2Dispatch(mTempBuff,0,length);
      		}
